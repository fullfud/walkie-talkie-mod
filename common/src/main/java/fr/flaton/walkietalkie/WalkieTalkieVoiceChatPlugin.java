...
import fr.flaton.walkietalkie.sound.ModSoundEvents;
...
public class WalkieTalkieVoiceChatPlugin implements VoicechatPlugin {
    ...
    // Вспомогательный метод, исправленный под твою версию API
    private PlayerEntity getPlayerByUuid(UUID uuid) {
        if (api == null) return null;
        // Этот способ перебора соединений работает в старых версиях API
        for (VoicechatConnection connection : api.getConnections()) {
            if (connection.getPlayer().getUuid().equals(uuid)) {
                return (PlayerEntity) connection.getPlayer().getPlayer();
            }
        }
        return null;
    }
